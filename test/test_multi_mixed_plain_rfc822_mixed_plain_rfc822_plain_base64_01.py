...
TEST_FILE = 'samples/email/multi_mixed_plain_rfc822_mixed_plain_rfc822_plain_base64_01.eml'
...
def test_message_headers():
    ...
    assert results[1]['headers']['to'][0].startswith(
        'John <john@csirtgadgets.org>')
    assert results[2]['headers']['to'][0].startswith(
        'John <john@csirtgadgets.org>')
...
def test_message_parts():
    ...
    assert results[1]['mail_parts'][0]['decoded_body'].startswith(
        'phishing message attached')
    assert results[2]['mail_parts'][0]['decoded_body'].startswith(
        'forward attachment of attachment')
